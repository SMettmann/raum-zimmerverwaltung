...
(function(){
  function setupPasswordUi(){
    ...
    if(cloud.mode==='online'&&cloud.user&&grid&&!document.getElementById('ownPasswordPanel')){
  ...
  window.openOwnPasswordDialog=function(){
    setupPasswordUi();hideFormError('changePasswordError');
    ['currentPassword','newPassword','repeatPassword'].forEach(id=>document.getElementById(id).value='');showModal('changePasswordModal');
  };
  ...
  window.openAdminPasswordDialog=function(id,name){
    ...
    setupPasswordUi();hideFormError('adminPasswordError');document.getElementById('adminPasswordUserId').value=id;document.getElementById('adminPasswordUserName').textContent=`Neues Passwort für ${name}`;document.getElementById('adminNewPassword').value='';document.getElementById('adminRepeatPassword').value='';showModal('adminPasswordModal');
  };
  ...
  loadCloudUsers=async function(){
    const res=await fetch('/api/users');if(!res.ok)return;const data=await res.json();const wrap=document.getElementById('cloudUserList');if(!wrap)return;const isAdmin=cloud.user?.role==='admin';
    wrap.innerHTML=(data.users||[]).map(u=>{const self=u.id===cloud.user?.id;const roleOptions=['admin','manager','staff','cleaning','viewer'].map(r=>`<option value="${r}" ${u.role===r?'selected':''}>${esc(roleLabel[r]||r)}</option>`).join('');return `<div class="cloud-admin-row password-enabled"><div><b>${esc(u.name)}</b><div class="muted">${esc(u.email)}${self?' · Du':''}</div></div><div>${isAdmin?`<select ${self?'disabled':''} onchange="changeCloudUserRole('${esc(u.id)}',this.value)">${roleOptions}</select>`:`<span class="badge">${esc(roleLabel[u.role]||u.role)}</span>`}</div><div>${isAdmin?`<button class="btn small ${u.active?'danger':''}" ${self?'disabled':''} onclick="toggleCloudUserActive('${esc(u.id)}',${u.active?'false':'true'})">${u.active?'Deaktivieren':'Aktivieren'}</button>`:`<span class="badge ${u.active?'green':'red'}">${u.active?'Aktiv':'Inaktiv'}</span>`}</div><div>${isAdmin?`<button class="btn small" onclick="openAdminPasswordDialog('${esc(u.id)}','${esc(u.name).replace(/'/g,'&#39;')}')">${self?'Eigenes ändern':'Passwort setzen'}</button>`:''}</div></div>`}).join('')||'<div class="empty">Keine Benutzer vorhanden.</div>';
  };
